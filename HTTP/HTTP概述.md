...
HTTP 协议设计与20世纪90年代初期，是一种可拓展的应用层协议。通过TCP，或者TLS 加密的TCP 连接来发送。

## 基于HTTP的组件系统

HTTP是一个client-server 协议：请求通过一个实体发出，这个实体就是用户代理，大多数情况下这个代理都是浏览器，他可能是任何东西，比如一个爬取网页生成维护搜索引擎的机器爬虫。

每一个请求被发出后，服务器都会返回一个 *response* ，在请求与相应之间，还有许多被称为 **proxies** 的实体，这些实体担任不同的角色，发挥不同的作用，比如有些是网关，有些是 <u>caches</u> 等。
